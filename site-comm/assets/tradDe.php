<?php
$trad = [
	"langue" => [
		"abrevia" => "de",
		"drap" => "🇩🇪"
	],
	"header" => [
		"alt" => "Logo von AlsAgriNet",
		"slogan" => "Ihr Partner für nachhaltige Landwirtschaft"
	],
	"pres" => [
		"titre" => "Willkommen bei AlsAgriNet",
		"contenu" => "Wir setzen uns dafür ein, innovative und nachhaltige Lösungen für die Landwirtschaft bereitzustellen.
					<br/>
					Zu diesem Zweck ermöglichen Ihnen unsere vernetzten Feldlösungen,
					Ihre Pflanzen in Echtzeit zu überwachen und die
					richtigen Entscheidungen zum richtigen Zeitpunkt zu treffen."
	],
	"serv" => [
		"titre" => "Unsere Dienstleistungen",
		"contenu" => "<li>Verkauf von Sensorsätzen</li>
					<ul>
						<li>Auswahl des Mikrocomputers</li>
						<li>Die auf Ihre Bedürfnisse zugeschnittenen Sensoren</li>
						<li>Software, die zu Ihrem Projekt passt</li>
					</ul>
				<li>Lösung zur Überwachung der Messungen der Sensoren über das Internet</li>
				<li>Installation von Sensoren</li>
					<ul>
						<li>Direkt auf dem Feld</li>
						<li>Erläuterungen zur Wartung</li>
					</ul>
				<li>Wartung von Sensoren</li>
					<ul>
						<li>Wartungsdienst</li>
						<li>Reparaturservice</li>
						<li>Beratung bei Problemen</li>
					</ul>
				<li>Rundum-Angebot für ein vernetztes Feld</li>
					<ul>
						<li>Bereitstellung von Sensoren, die zu Ihrem Projekt passen</li>
						<li>Eine Website, um Ihre Felder zu verfolgen</li>
						<li>Wartungsdienst</li>
					</ul>",
		"alt" => "Foto eines Raspberry Pi 4"
	],
	"apropos" => [
		"titre" => "Über uns",
		"contenu" => "AlsAgriNet ist ein Unternehmen mit Sitz im Elsass, das sich auf
					die Entwicklung von vernetzten Feldern spezialisiert hat.
					Mit 8 Jahren Erfahrung haben wir uns
					als führend in nachhaltiger Landwirtschaft im
					Elsass etabliert.",
		"equipe" => [
			"titre" => "Unser Team",
			"contenu" => "Wir sind ein Team von 9 Personen, die sich leidenschaftlich für
						Landwirtschaft, Technologie und Innovation interessieren.
						<br/>
						Immer aufmerksam für unsere Kunden sind wir
						verpflichtet, Lösungen anzubieten, die auf Ihre
						Bedürfnisse zugeschnitten sind.",
			"alt" => "Foto des AlsAgriNet-Teams"
		],
		"rs" => [
			"titre" => "Unsere sozialen Medien",
			"alt" => [
				"fb" =>  "Facebook-Symbol",
				"x" => "X-Symbol",
				"lkdn" => "LinkedIn-Symbol"
			]
		]
	],
	"contact" => [
		"courriel" => "E-Mail-Adresse:",
		"telephone" => "Telefonnummer:",
		"adresse" => "Adresse:",
		"horaires" => "Öffnungszeiten:",
		"jours" => [
			1 => "Montag: ",
			2 => "Dienstag: ",
			3 => "Mittwoch: ",
			4 => "Donnerstag: ",
			5 => "Freitag: ",
			6 => "Samstag: ",
			7 => "Sonntag: ",
			"ferme" => "Geschlossen",
		]
	],
	"bulle" => [
		"altBulle" => "Fragezeichensymbol",
		"altEnt" => "Kreuzsymbol",
		"titre" => "Kontaktiere uns",
		"form" => [
			"nom" => "Name",
			"prenom" => "Vorname",
			"courriel" => "E-Mail",
			"phCourriel" => "adresse@email.de",
			"message" => "Nachricht",
			"phMessage" => "Ihre Anfrage...",
			"envoi" => "Senden"
		],
		"retour" => [
			0 => "<p>Ihre Nachricht wurde erfolgreich gesendet!</p>",
			1 => "<p>Beim Senden der Nachricht ist ein Fehler aufgetreten.</p>",
			2 => "<p>Beim Senden der Nachricht ist ein Fehler aufgetreten.
				Bitte versuchen Sie es später erneut.</p>"
		]
	],
	"footer" => [
		"droits" => "Alle Rechte vorbehalten"
	]
];